$(document).ready(function() {
    $.ajax({
        type: "GET",
        url: "../src/scraper/piazza_scrape.csv",
        dataType: "text",
        success: function(data) {piazza(data);}
     });
});

function piazza(allText) {
...
$(document).ready(function() {
    $.ajax({
        type: "GET",
        url: "../src/scraper/fundies_scrape.csv",
        dataType: "text",
        success: function(data) {hwLab(data);}
     });
});

function hwLab(allText) {
    var allTextLines = allText.split(/\r\n|\n/);
    var headers = allTextLines[0].split();
    var lines = [];
